...
export default function VehicleCard({ vehicle, className = '', width }: VehicleCardProps) {
  ...
  return (
    <div 
      className={cn(
        "group relative overflow-hidden bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-blue-400 dark:hover:border-blue-500 transition-all duration-300 h-[420px] sm:h-[450px] flex flex-col",
      ...
      <div className="p-4 flex flex-col flex-1 justify-between">
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-3 my-4">
          ...
          <div className="flex flex-col justify-center items-start rounded-lg p-2.5 bg-green-50 dark:bg-green-900/30 border border-green-100 dark:border-green-800/50">
            ...
            <span className={`text-sm font-bold ${(vehicle.profit ?? 0) < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-white'}`}>
              {(() => {
                console.log('Rendering profit value:', vehicle.profit);
                const profitValue = vehicle.profit;
                if (profitValue === undefined) return '--';
                const formattedValue = Math.abs(Math.round(profitValue)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                return `${profitValue < 0 ? '-' : ''}$${formattedValue}`;
              })()}
            </span>
          </div>
